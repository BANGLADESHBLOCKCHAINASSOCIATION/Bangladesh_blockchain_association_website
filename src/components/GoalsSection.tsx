...
export default function GoalsSection() {
	...
	return (
		<div className="py-14">
			...
			<MotionDiv variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }} className="flex gap-10 flex-row-reverse">
				<MotionDiv variants={item} className="w-1/2 hidden lg:flex flex-col justify-center gap-5 p-14 border rounded-md h-fit">
				...
				<div className="w-full lg:w-1/2 border rounded-md overflow-hidden">
					{goals?.map((goal, index) => (
						<MotionDiv variants={item} onClick={() => setGoalIndex(index)} key={goal.title} className="border-b last:border-b-0  p-8 hover:bg-[#8ad29f] cursor-pointer gap-5">
